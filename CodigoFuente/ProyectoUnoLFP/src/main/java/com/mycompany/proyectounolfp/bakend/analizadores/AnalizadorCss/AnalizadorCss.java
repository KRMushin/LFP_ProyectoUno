...
import com.mycompany.proyectounolfp.bakend.analizadores.AnalizadorCss.AutomatasCss.OtrosCss;
import com.mycompany.proyectounolfp.bakend.analizadores.AnalizadorCss.AutomatasCss.ReglasCss;
            ...
            switch (estadoActual) {
                case Q0:
                    if (Character.isLetter(caracter)) {
                        ...
                        estadoActual = ESTADO.ETIQUETA_REGLA;
                            ...
                            lexemaActual.append(caracter);
                            tokensCss.add(new Token(TipoTokenCss.COMBINADOR, lexemaActual.toString()));
                        ...
                        lexemaActual.append(caracter);
                        tokensCss.add(new Token(TipoTokenCss.UNIVERSAL, lexemaActual.toString()));
                        lexemaActual.setLength(0);
                        ultimoTokenEsSelector = false;
                        ...
                        tokensCss.add(new Token(TipoTokenCss.CARACTERES_VALOR, String.valueOf(caracter)));
                    } else if (caracter == ':' && contenido[i + 1] == ':'){
                        tokensCss.add(new Token(TipoTokenCss.CARACTERES_VALOR, "::"));
                    } else if (caracter == '\'') {
                        lexemaActual.append(caracter);
                        ...
                        tokensCss.add(new Token(TipoTokenCss.ERROR, String.valueOf(caracter)));
                    }
                    break;
                ...
                case ETIQUETA_REGLA:
                    ...
                    if (Character.isLetterOrDigit(caracter) || caracter == '-' || caracter == '('){
                        ...
                        if (EtiquetasCss.esEtiquetaCss(lexemaActual.toString())){
                            tokensCss.add(new Token(TipoTokenCss.ETIQUETA, lexemaActual.toString()));
                            lexemaActual.setLength(0);  // Limpiar el lexema
                            ultimoTokenEsSelector = true;
                            estadoActual = ESTADO.Q0;
                        } else if (ReglasCss.esReglaValida(lexemaActual.toString())){
                            tokensCss.add(new Token(TipoTokenCss.REGLAS_CSS, lexemaActual.toString()));
                            ...
                            lexemaActual.setLength(0);  // Limpiar el lexema
                            estadoActual = ESTADO.Q0;
                        } else if (OtrosCss.esOtroCss(lexemaActual.toString())) {
                            tokensCss.add(new Token(TipoTokenCss.CARACTERES_VALOR, lexemaActual.toString()));
                            ...
                            tokensCss.add(new Token(TipoTokenCss.IDENTIFICADOR, lexemaActual.toString()));
                            lexemaActual.setLength(0);  // Limpiar el lexema
                            estadoActual = ESTADO.Q0;

                        } else {
                            tokensCss.add(new Token(TipoTokenCss.ERROR, lexemaActual.toString()));
                            lexemaActual.setLength(0);  // Limpiar el lexema
                            estadoActual = ESTADO.Q0;
                        }
                    } else{
                    ...
                    if(Character.isLetter(caracter) || Character.isDigit(caracter) || caracter == '-'){
                        ...
                        if (automataClase.esClaseValida(lexemaActual.toString())){
                            if (lexemaActual.toString().startsWith("#")){
                                tokensCss.add(new Token(TipoTokenCss.DE_ID, lexemaActual.toString()));
                            }else{
                                tokensCss.add(new Token(TipoTokenCss.DE_CLASE, lexemaActual.toString()));
                            }
                        }else{
                            tokensCss.add(new Token(TipoTokenCss.ERROR, lexemaActual.toString()));
                        }
                ...
                case CADENA:
                        if (Character.isLetterOrDigit(caracter) || esSignoSimbolo(caracter)){
                            ...
                            lexemaActual.append(caracter);
                            tokensCss.add(new Token(TipoTokenCss.CADENA, lexemaActual.toString()));
                            lexemaActual.setLength(0);
                            estadoActual = ESTADO.Q0;
                        } else{
                            tokensCss.add(new Token(TipoTokenCss.ERROR, lexemaActual.toString()));
                    ...
                    if(caracter == 'f'){
                        ...
                        //permacer en este estado
                            ...
                            if (lexemaActual.toString().length() == 3 || lexemaActual.toString().length() == 6) {
                                tokensCss.add(new Token(TipoTokenCss.COLOR, lexemaActual.toString()));
                                lexemaActual.setLength(0);
                            }else{
                                tokensCss.add(new Token(TipoTokenCss.ERROR, lexemaActual.toString()));
                                lexemaActual.setLength(0);
                ...
                case COLOR_RGBA:
                     if (Character.isLetterOrDigit(caracter) || caracter == ' ' || caracter == '(' || caracter == ','){
                        ...
                        if (contador == 1){
                            ...
                            lexemaActual.append(caracter);
                            tokensCss.add(new Token(TipoTokenCss.COLOR_RGBA, lexemaActual.toString()));
                            lexemaActual.setLength(0);
                            estadoActual = ESTADO.Q0;
                     }
                     else{
